from tok import Token, TokenType, lookup_ident
...
class Lexer:
    ...
    def __read_number(self) -> Token:
        ...
        while self.__is_digit(self.current_char) or self.current_char == '.':
            ...
            if dot_count > 1:
                print("Too many decimals in number on line {self.line_no}, position {self.position}")
    ...
    def next_token(self) -> Token:
        ...
        match self.current_char:
            ...
            case '*':
                tok = self.__new_token(TokenType.MUL, self.current_char)
            case '/':
                tok = self.__new_token(TokenType.DIVIDE, self.current_char)
            case '^':
                tok = self.__new_token(TokenType.POW, self.current_char)
            case '%':
                tok = self.__new_token(TokenType.PERCENT, self.current_char)
            case '=':
            ...
            case _:
                if self.__is_letter(self.current_char):
                    ...
                    tok = self.next_token(TokenType.ILLEGAL, self.current_char)
